...
Status VFS::read_seg_info(){
    ...
    for(int i = 0; i < nums; i++,j+=8){
        ...
        uint64_t p_addr = int8_to_int64(result, j+4);
...
Status VFS::write_add_seg_info(){
    ...
    int findex =8;
    for(auto item:(M)){
        temp = int64_to_int8(item.first);
        for(int i = 0; i< 8;i++) input[findex+i] = temp[i];
        findex += 8;
        temp = int64_to_int8(item.second);
        for(int i = 0; i< 8;i++) input[findex+i] = temp[i];
        findex += 8;
    }
...
Status VFS::read_seg_head(BlockHandle *bh, SegHead **sh){
    ...
    *sh = new SegHead();
    ...
    (*sh)->meta_first_addr = int8_to_int64(result,4);//反序列首元数据地址
    return s;
}
...
Status VFS::write_seg_head(BlockHandle *bh, SegHead *sh){
    ...
    temp = int64_to_int8(sh->meta_first_addr);
...
Status VFS::read_seg_meta(BlockHandle *bh, SegMeta **sm){
    ...
    *sm = new SegMeta();
    ...
    (*sm)-> next_meta_addr = int8_to_int64(result,4);
    int j = 12;
    for(int i = 0; i < VFS::META_NUMS; i++){
        uint32_t state = int8_to_int32(result,j);
        j+=4;
        uint64_t p_addr = int8_to_int64(result, j);
        j+=8;
...
Status VFS::write_seg_meta(BlockHandle *bh, SegMeta *sm){
    ...
    for(int i = 0;i<4;i++) result[i] = temp[i];
    temp = int64_to_int8(sm->next_meta_addr);
    for(int i = 0;i<8;i++) result[4+i] = temp[i];
    int findex = 12;
    for(auto item:sm->M){
